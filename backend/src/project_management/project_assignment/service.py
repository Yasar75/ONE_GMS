...
class ProjectAssignmentService:
    ...
    async def _ensure_project_is_active(
        self,
        db: AsyncSession,
        project_uid: uuid.UUID,
    ) -> Project:
        project = await self._get_project_or_404(db, project_uid)
    ...
    def _is_date_overlap(
        self,
        assigned_from: Optional[date],
        assigned_to: Optional[date],
        existing_from: Optional[date],
        existing_to: Optional[date],
    ) -> bool:
        ...
        *,
        ...
        for item in assignments:
            if self._is_date_overlap(
                ...
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Employee already has an overlapping assignment for this project",
                )

    async def _sync_employee_billing_status(
        self,
        db: AsyncSession,
        employee_uid: uuid.UUID,
    ) -> None:
    ...
    async def create_project_assignment(
        ...
        await self._ensure_project_is_active(db, payload.project_uid)
    ...
    async def get_project_assignment_by_uid(
        self,
        db: AsyncSession,
    ...
    async def list_project_assignments(
        ...
        *,
        ...
        if search:
            ...
            conditions.append(
                or_(
                    ProjectAssignment.pod_name.ilike(search_value),
                    ProjectAssignment.team_lead.ilike(search_value),
                    ProjectAssignment.status.ilike(search_value),
                    ProjectAssignment.remarks.ilike(search_value),
                )
            )
        ...
        if status_filter:
            conditions.append(
                func.lower(ProjectAssignment.status) == status_filter.strip().lower()
            )
    ...
    async def update_project_assignment(
        ...
        await self._ensure_project_is_active(db, new_project_uid)
        ...
        if new_assigned_from and new_assigned_to and new_assigned_to < new_assigned_from:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="assigned_to cannot be earlier than assigned_from",
            )
