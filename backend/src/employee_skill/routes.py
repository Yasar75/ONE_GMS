...
admin_module= "Employee Skills"
employee_admin_module= "My Skills"

##Get All skill
@employee_skill_router.get("/", response_model=List[EmployeeSkillRead], status_code=status.HTTP_200_OK,dependencies=[Depends(PermissionChecker(admin_module, "r"))])
...
@employee_skill_router.post("", status_code=status.HTTP_201_CREATED, response_model=EmployeeSkillRead, dependencies=[Depends(PermissionChecker(employee_admin_module, "c"))])
...
@employee_skill_router.put("/{skill_uid}", response_model=EmployeeSkillRead, dependencies=[Depends(PermissionChecker(employee_admin_module, "u"))])
...
@employee_skill_router.delete("/{skill_uid}", status_code=status.HTTP_200_OK, dependencies=[Depends(PermissionChecker(employee_admin_module, "d"))])
