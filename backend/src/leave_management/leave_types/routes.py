...
async def create_leave_type(data: LeaveTypeCreate,session: AsyncSession = Depends(get_session),token_details: dict = Depends(access_token_bearer)):
...
async def get_leave_type(leave_type_uid: uuid.UUID,session: AsyncSession = Depends(get_session)):
...
async def update_leave_type(leave_type_uid: uuid.UUID,data: LeaveTypeUpdate,session: AsyncSession = Depends(get_session)):
    return await leave_type_service.update_leave_type(session, leave_type_uid, data)


@leave_type_router.delete("/{leave_type_uid}",status_code=status.HTTP_200_OK,dependencies=[Depends(PermissionChecker(module, "d"))])
async def delete_leave_type(leave_type_uid: uuid.UUID,session: AsyncSession = Depends(get_session)):
