...
export const PROJECT_IMPORT_TEMPLATE_SAMPLE_ROWS = [
  ['PRJ-1001', 'One GMS Core Platform', 'Active', '2025-01-15', '2026-01-15', 'Core product roadmap execution'],
  ['PRJ-1002', 'Analytics Dashboard Revamp', 'Planned', '15/04/2026', '15/08/2026', 'Reporting and visual refresh']
]
...
export function downloadProjectsAsCsv(records = []) {
  const headers = ['Project Code', 'Project Name', 'Status', 'Start Date', 'End Date', 'Description']
  const rows = (Array.isArray(records) ? records : []).map((project) => ([
    project.projectCode || '',
    project.projectName || '',
    project.status || '',
    project.startDate || '',
  ...
  downloadBlob(blob, `project-directory-${Date.now()}.csv`)
}

export function downloadProjectsAsExcel(records = []) {
  const headers = ['Project Code', 'Project Name', 'Status', 'Start Date', 'End Date', 'Description']
  const rows = (Array.isArray(records) ? records : []).map((project) => ([
    project.projectCode || '',
    project.projectName || '',
  ...
  downloadBlob(blob, `project-directory-${Date.now()}.xlsx`)
}
...
export function downloadProjectImportTemplateCsv() {
  const csvContent = serializeRowsToCsv([PROJECT_IMPORT_HEADERS, ...PROJECT_IMPORT_TEMPLATE_SAMPLE_ROWS])
  const blob = new Blob([`${csvContent}\n`], { type: 'text/csv;charset=utf-8;' })
  downloadBlob(blob, 'project-import-template.csv')
}

export function downloadProjectImportTemplateExcel() {
  const workbook = XLSX.utils.book_new()
  const worksheet = XLSX.utils.aoa_to_sheet([PROJECT_IMPORT_HEADERS, ...PROJECT_IMPORT_TEMPLATE_SAMPLE_ROWS])
  XLSX.utils.book_append_sheet(workbook, worksheet, 'Project Import')
  const workbookBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' })
  const blob = new Blob([workbookBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
  downloadBlob(blob, 'project-import-template.xlsx')
}
